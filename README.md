...
LINK : https://cloudops-sentinel-copilot.onrender.com
...
├── requirements.txt
├── Dockerfile
└── .env                       # you create this yourself (see Setup)
```

## 1. Setup

Requires **Python 3.11 or 3.12** (Python 3.13/3.14 are not yet compatible with some dependencies, notably `langchain-pinecone`).
...
Create a file named `.env` in the project root (there is no `.env.example` shipped with this project) with:
...
The defaults for everything else (already set in `src/config.py`, override in `.env` only if needed):
...
This starts the app on **port 8080** (set in `app.py`'s `uvicorn.run(..., port=8080, ...)`). Open:

```text
http://127.0.0.1:8080
```

Alternatively, you can run it via the uvicorn CLI directly, which defaults to port 8000 instead:
...
Either way, check your terminal output for the "Uvicorn running on ..." line to confirm which port is actually in use.
